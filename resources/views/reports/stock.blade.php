    ...
    <div class="space-y-4">
        <div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
            <div class="mb-4 flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
                ...
                <a href="{{ route('reports.stock', array_merge($filters, ['export' => 'csv'])) }}" class="w-full rounded-lg border border-emerald-300 px-4 py-2.5 text-center text-sm font-medium text-emerald-700 hover:bg-emerald-50 sm:w-auto">Export CSV</a>
            </div>

            <form method="GET" action="{{ route('reports.stock') }}" class="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                <div>
                ...
                <div class="flex items-end gap-2 md:col-span-2 xl:col-span-2">
                    ...
                    <a href="{{ route('reports.stock') }}" class="h-11 w-full rounded-lg border border-slate-300 px-4 text-center text-sm leading-[2.75rem] sm:w-auto">Reset</a>
                </div>
            </form>
        </div>
                    ...
                    <tbody>
                        @forelse($products as $product)
                            <tr class="block border-b border-slate-100 py-2 md:table-row md:py-0">
                                ...
                                <td class="flex items-center justify-between px-3 py-1.5 font-semibold {{ $product->current_stock <= $product->minimum_stock ? 'text-red-600' : 'text-slate-900' }} md:table-cell md:py-3">
                                ...
                                <td class="flex items-center justify-between px-3 py-1.5 md:table-cell md:py-3">
                                    ...
                                    <span class="rounded-full px-2 py-1 text-xs font-medium {{ $product->current_stock <= $product->minimum_stock ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700' }}">{{ $product->current_stock <= $product->minimum_stock ? 'Low Stock' : 'OK' }}</span>
                                </td>
                            </tr>
                        @empty
                            <tr><td colspan="6" class="px-3 py-8 text-center text-slate-500">No stock data found.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
